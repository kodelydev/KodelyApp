...
const TestComponent = () => {
	const { allowedCommands, setAllowedCommands, soundEnabled, showKodelyIgnoredFiles, setShowKodelyIgnoredFiles } =
		useExtensionState()
	return (
		<div>
			<div data-testid="allowed-commands">{JSON.stringify(allowedCommands)}</div>
			<div data-testid="sound-enabled">{JSON.stringify(soundEnabled)}</div>
			<div data-testid="show-kodelyignored-files">{JSON.stringify(showKodelyIgnoredFiles)}</div>
			<button data-testid="update-button" onClick={() => setAllowedCommands(["npm install", "git status"])}>
				Update Commands
			</button>
			<button data-testid="toggle-kodelyignore-button" onClick={() => setShowKodelyIgnoredFiles(!showKodelyIgnoredFiles)}>
				Update Commands
			</button>
		</div>
	)
...
describe("ExtensionStateContext", () => {
	...
	it("initializes with showKodelyIgnoredFiles set to true", () => {
		render(
			<ExtensionStateContextProvider>
				<TestComponent />
			</ExtensionStateContextProvider>,
		...
		expect(JSON.parse(screen.getByTestId("show-kodelyignored-files").textContent!)).toBe(true)
	})

	it("updates showKodelyIgnoredFiles through setShowKodelyIgnoredFiles", () => {
		render(
			<ExtensionStateContextProvider>
				<TestComponent />
			</ExtensionStateContextProvider>,
		...
		act(() => {
			screen.getByTestId("toggle-kodelyignore-button").click()
		})

		expect(JSON.parse(screen.getByTestId("show-kodelyignored-files").textContent!)).toBe(false)
	})
